...
ConsumptionLog _log(String id, String itemId, {String? labId}) =>
    ConsumptionLog(
      id: id,
      itemId: itemId,
...
PendingOperation _op(String id, Map<String, dynamic> payload) =>
    PendingOperation(
      id: id,
      userId: 'me',
      type: 'add_chemical',
...
void main() {
  ...
  test('a team lab shows only that lab, including a legacy log', () {
    ...
    expect(scoped.logs.map((log) => log.id), [
      'team-log',
      'legacy-team-log',
      'orphan-team-log',
    ]);
  ...
  test('a chemical map omits lab id until one is set', () {
    ...
    expect(Chemical.fromMap({'id': 'x', 'lab_id': 'lab-a'}).labId, 'lab-a');
  });
